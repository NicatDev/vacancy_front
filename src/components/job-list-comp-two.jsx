...
import {LuMapPin, AiOutlineClockCircle, MdKeyboardArrowLeft, MdKeyboardArrowRight,AiOutlineStar,FiBookmark} from "../assets/icons/vander"
        ...
        <div className="lg:col-span-8 md:col-span-6">
            <div className="grid grid-cols-1 gap-[30px]">
                ...
                {jobData.map((item, index) => (
                    <div key={index} className="group relative overflow-hidden lg:flex justify-between items-center rounded shadow-sm hover:shadow-md  shadow-gray-200 dark:shadow-gray-700 transition-all duration-500 p-5">
                        <div className="flex items-center">
                            <div
                                className="size-14 flex items-center justify-center bg-white dark:bg-slate-900 shadow-sm shadow-gray-200 dark:shadow-gray-700 rounded-md">
                                <img src={item.image} className="size-8" alt="" />
                            </div>
                            <Link to="/job-detail-three"
                                className="text-lg hover:text-emerald-600 font-semibold transition-all duration-500 ms-3 min-w-[150px]">{item.heading}</Link>
                        </div>
                        ...
                        <div className="lg:mt-0 mt-4">
                            <Link to="#"
                                className="size-9 font-semibold tracking-wide border align-middle transition duration-500 ease-in-out inline-flex items-center text-center justify-center text-base rounded-full bg-emerald-600/5 hover:bg-emerald-600 border-emerald-600/10 hover:border-emerald-600 text-emerald-600 hover:text-white lg:relative absolute top-0 end-0 lg:m-0 m-3">
                                <FiBookmark className="size-4"></FiBookmark></Link>
                            <Link to="/job-apply"
                                className="py-1 px-5 inline-block font-semibold tracking-wide border align-middle transition duration-500 ease-in-out text-base text-center rounded-md bg-emerald-600 hover:bg-emerald-700 border-emerald-600 hover:border-emerald-700 text-white md:ms-2 w-full md:w-auto">{item.apply}</Link>
                        </div>
