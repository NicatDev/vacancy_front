...
export default function CandidateDetail() {
  ...
  const handleDownloadCV = async (e) => {
    ...
    try {
      ...
      const message = t("common.fileNotFound");
      toast.error(message);
    }
  };
  ...
  const getSkills = async () => {
    try {
      if (!user) return;
      let response = await CandidatesAPI.getSingleCandidateSkills(
        user?.data?.id
      );
  ...
  const getApplications = async () => {
    try {
      if (!user) return;
      let response = await CandidatesAPI.getCandidateApplications(
        user?.data?.id
      );
                    ...
                    {languages?.length > 0 ? (
                      languages.map((language) => (
                        <div
                          ...
                          className="flex items-center gap-2 justify-between mb-2"
                        >
                          <span className="text-slate-400">
            ...
            <div className="lg:col-span-4 md:col-span-5">
              <div className="bg-slate-50 dark:bg-slate-800 rounded-md shadow-sm shadow-gray-200 dark:shadow-gray-700 p-6 sticky top-20">
                ...
                <ul className="list-none mt-4">
                  ...
                  <li className="flex justify-between mt-3 items-center font-medium">
                    <span className="text-slate-400 me-3">
                      {t("candidateProfile.salaryExpectation")} :
                    </span>
                  ...
                  <li className="flex justify-between mt-3 items-center font-medium">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-slate-400 me-3">
                        {t("candidateProfile.email")} :
                      </span>
                    </div>
                    <span className="email-text">{user?.data?.user?.email}</span>
                  </li>
